...
public class PrayEvent {
    ...
    private static Block findShrineBlock() {
        if (SHRINE_BLOCK == null) {
            SHRINE_BLOCK = BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("touhou_little_maid", "shrine"));
        }
        return SHRINE_BLOCK;
    }
    ...
    private static void sendPrayState(ServerLevel level, EntityMaid maid, boolean praying) {
        PacketDistributor.sendToPlayersTrackingEntity(maid, new PraySyncPacket(maid.getId(), praying));
    }
}
